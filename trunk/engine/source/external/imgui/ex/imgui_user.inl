...
namespace ImGuiEx 
{
	...
	void Rect(float w, float h, ImU32 color, bool mouse_input)
	{
		ImVec2 start_pos = ImGui::GetCursorScreenPos();
		Rect(w, h, color);
		if (!mouse_input)
		{
			ImGui::SetNextWindowPos(start_pos);
			ImGui::SetNextWindowSize(ImVec2(w, h));
			ImGui::Begin("Mask", 0, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoBackground);
			ImGui::End();
		}
	}
	...
	bool Gradient4(const char* label, int max_count, int* count, float* keys, float* values) {
		...
		ImVec2 max = min + ImVec2(w, GetTextLineHeight());
...
//		NewLine();
	...
	bool CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
	{
		...
		if (p_visible != NULL)
		{
			...
			char sz_name[256] = { 0 };
			snprintf(sz_name, IM_ARRAYSIZE(sz_name), "##check_%s", label);
			ImGui::Checkbox(sz_name, p_visible);
