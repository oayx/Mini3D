...
void ___final_insertion_sort(RandomAccessIterator first, RandomAccessIterator last, Compared comp);
template <class RandomAccessIterator>
void __insertion_sort(RandomAccessIterator first, RandomAccessIterator last);
template <class RandomAccessIterator, class Compared>
void ___insertion_sort(RandomAccessIterator first, RandomAccessIterator last, Compared comp);
...
OutputIterator merge(InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2, OutputIterator result, Compared comp);
...
void ___merge_adaptive(BidirectionalIterator first, BidirectionalIterator middle, BidirectionalIterator last, Distance len1, Distance len2, Pointer buffer, Distance buffer_size, Compared comp);
template <class BidirectionalIterator, class Distance, class Compared>
void ___merge_without_buffer(BidirectionalIterator first, BidirectionalIterator middle, BidirectionalIterator last, Distance len1, Distance len2, Compared comp);
...
inline ForwardIterator lower_bound(ForwardIterator first, ForwardIterator last, const T& value, Compared comp);
...
ForwardIterator1 search(ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2, ForwardIterator2 last2, Compared comp);
template <class ForwardIterator1, class ForwardIterator2>
ForwardIterator2 swap_ranges(ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2);
template <class ForwardIterator, class T>
ForwardIterator upper_bound(ForwardIterator first, ForwardIterator last, const T& value);
template <class ForwardIterator, class T, class Compared>
inline ForwardIterator upper_bound(ForwardIterator first, ForwardIterator last, const T& value, Compared comp);
...
RandomAccessIterator __unguarded_partition(RandomAccessIterator first, RandomAccessIterator last, T pivot, Compared comp);
template <class InputIterator, class OutputIterator>
OutputIterator unique_copy(InputIterator first, InputIterator last, OutputIterator result);
template <class RandomAccessIterator, class T>
void __unguarded_linear_insert(RandomAccessIterator last, T value);
template <class RandomAccessIterator, class T, class Compared>
void ___unguarded_linear_insert(RandomAccessIterator last, T value, Compared comp);
/*****************************************************************************************/
// adjacent_find
...
void ___merge_without_buffer(BidirectionalIterator first, BidirectionalIterator middle,BidirectionalIterator last, Distance len1, Distance len2, Compared comp) {
    ...
    auto new_middle = tinystl::rotate(first_cut, middle, second_cut);
    // 递归处理
	___merge_without_buffer(first, first_cut, new_middle, len11, len22, comp);
	___merge_without_buffer(new_middle, second_cut, last, len1 - len11, len2 - len22, comp);
}
...
void ___merge_adaptive(BidirectionalIterator first, BidirectionalIterator middle,BidirectionalIterator last, Distance len1, Distance len2, Pointer buffer, Distance buffer_size, Compared comp) {
    ...
    if (len1 <= len2 && len1 <= buffer_size) {
        ...
        // 递归处理
		___merge_adaptive(first, first_cut, new_middle, len11, len22, buffer, buffer_size, comp);
		___merge_adaptive(new_middle, second_cut, last, len1 - len11,len2 - len22, buffer, buffer_size, comp);
    }
}
...
inline void ___inplace_merge_aux(BidirectionalIterator first, BidirectionalIterator middle,BidirectionalIterator last, T*, Distance*, Compared comp) {
	auto len1 = distance(first, middle);
	auto len2 = distance(middle, last);
	temporary_buffer<BidirectionalIterator, T> buf(first, last);  // 申请额外缓冲区
	if (!buf.begin())  // 申请失败   
		___merge_without_buffer(first, middle, last, len1, len2, comp);
	else
		___merge_adaptive(first, middle, last, len1, len2, buf.begin(), buf.size(), comp);
}
...
inline void inplace_merge(BidirectionalIterator first, BidirectionalIterator middle,
	...
	___inplace_merge_aux(first, middle, last, value_type(first), distance_type(first), comp);
}
template <class BidirectionalIterator>
inline void inplace_merge(BidirectionalIterator first, BidirectionalIterator middle,
...
bool next_permutation(BidirectionalIterator first, BidirectionalIterator last) {
    ...
    for (;;) {
        ...
        if (i == first) {                 // 全部逆向了，没有下一组合
            tinystl::reverse(first, last);  // 全部反转
            return false;
        }
    }
}
...
void __nth_element(RandomAccessIterator first, RandomAccessIterator nth, 
    ...
    if (nth == last)  return;
    while (last - first > 3) {
        auto cut = tinystl::__unguarded_partition(first, last,
    ...
    tinystl::__insertion_sort(first, last);
}
...
void __nth_element(RandomAccessIterator first, RandomAccessIterator nth,
    ...
    tinystl::___insertion_sort(first, last, comp);
}
...
void __partial_sort(RandomAccessIterator first, RandomAccessIterator middle,
    ...
    for (auto i = middle; i < last; ++i) {
        if (*i < *first)  // max-heap 第一个元素最大
            tinystl::__pop_heap_aux(first, middle, i, static_cast<T>(*i), distance_type(first));
    }
...
void __partial_sort(RandomAccessIterator first, RandomAccessIterator middle,
	...
	for (auto i = middle; i < last; ++i) {
		if (comp(*i, *first))
			tinystl::__pop_heap_aux(first, middle, i, static_cast<T>(*i), distance_type(first), comp);
	}
...
RandomAccessIterator __partial_sort_copy(InputIterator first, InputIterator last,
    ...
    while (first != last) {
        if (*first < *result_first)
            tinystl::__adjust_heap(result_first, static_cast<Distance>(0),
...
RandomAccessIterator __partial_sort_copy(InputIterator first, InputIterator last,
    ...
    while (first != last) {
        if (comp(*first, *result_first))
            tinystl::___adjust_heap(result_first, static_cast<Distance>(0),result_iter - result_first, *first, comp);
        ++first;
    }
...
void __random_shuffle(RandomAccessIterator first, RandomAccessIterator last, Distance*) {
    ...
    for (auto i = first + 1; i != last; ++i) {
        tinystl::iter_swap(i, first + static_cast<Distance>(rand() % ((i - first) + 1)));
    }
}
...
ForwardIterator search_n(ForwardIterator first, ForwardIterator last, Size n, const T& value) {
    if (n <= 0) {
        ...
        first = tinystl::find(first, last, value);  // 先找出第一个 value 出现的位置
        while (first != last) {
...
RandomAccessIterator __unguarded_partition(RandomAccessIterator first, RandomAccessIterator last, T pivot) {
	while (true) {
		...
		tinystl::iter_swap(first, last);       // 交换 first 和 last 所指元素
		++first;
	}
}
...
inline void __linear_insert(RandomAccessIterator first, RandomAccessIterator last, T*) {
	...
	if (value < *first) {                             // 若要插入的值比头部元素还小
		tinystl::copy_backward(first, last, last + 1);  // 直接把整个区间后移一位
...
inline void __linear_insert(RandomAccessIterator first, RandomAccessIterator last, T*, Compared comp) {
    ...
    if (comp(value, *first)) {
        ...
        *first = value;
		...
		___unguarded_linear_insert(last, value, comp);
    }
}
...
void __unguarded_insertion_sort_aux(RandomAccessIterator first,
	RandomAccessIterator last, T*, Compared comp) {
	for (auto i = first; i != last; ++i) {                       // 外循环
		___unguarded_linear_insert(i, static_cast<T>(*i), comp);  // 内循环
	}
}

// 插入排序函数 __unguarded_insertion_sort
template <class RandomAccessIterator, class Compared>
inline void ___unguarded_insertion_sort(RandomAccessIterator first,RandomAccessIterator last, Compared comp) {
...
void ___final_insertion_sort(RandomAccessIterator first, RandomAccessIterator last, Compared comp) {
	if (static_cast<size_t>(last - first) > kSmallSectionSize) {  // 如果区间长度大于 kSmallSectionSize
		...
		___insertion_sort(first, first + kSmallSectionSize, comp);
		___unguarded_insertion_sort(first + kSmallSectionSize, last, comp);
	}
	else {
		___insertion_sort(first, last, comp);
	}
}
...
inline void sort(RandomAccessIterator first, RandomAccessIterator last, Compared comp) {
	if (first != last) {
		...
		___final_insertion_sort(first, last, comp);
	}
}
...
ForwardIterator unique(ForwardIterator first, ForwardIterator last) {
    first = tinystl::adjacent_find(first, last);    // 利用 adjacent_find 找到相邻重复元素的起点
...
ForwardIterator unique(ForwardIterator first, ForwardIterator last, Compared comp) {
    first = tinystl::adjacent_find(first, last, comp);    // 利用 adjacent_find 找到相邻重复元素的起点
