...
// 模板类 set
// 参数一代表键值类型，参数二代表实值类型，参数三代表键值的比较方式，缺省使用 tinystl 的 less
// 参数四代表空间配置器的类型，缺省使用 tinystl 的 alloc
...
class map {
  ...
  private:
    // 以 rb_tree 作为底层机制
    typedef rb_tree<key_type, value_type, tinystl::selectfirst<value_type>, key_compare, Alloc>  rep_type;
