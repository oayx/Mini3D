...
// 模板类 hash_set
// 参数一代表实值类型，参数二代表哈希函数，缺省使用 tinystl 的 hash
// 参数三代表键值比较方式，缺省使用 tinystl 的 equal_to，参数四代表空间配置器类型，缺省使用 tinystl 的 alloc
// 使用方法与 set 类似，以 hashtable 作为底层机制，所以 hash_set 中的元素不会自动排序
template <class Value, class HashFcn = tinystl::hash<Value>, 
    class EqualKey = tinystl::equal_to<Value>, class Alloc = alloc>
class hash_set {
  private:
    // 使用 hash_table 作为底层机制
    typedef hashtable<Value, Value, HashFcn, tinystl::identity<Value>, EqualKey, Alloc>  rep_type;
    rep_type ht_;
