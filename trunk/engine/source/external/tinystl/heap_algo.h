...
void ___adjust_heap(RandomAccessIterator first, Distance holeIndex,Distance len, T value, Compared comp) {
    // 先进行下溯(percolate down)过程
    auto topIndex = holeIndex;
    auto rchild = 2 * holeIndex + 2;  // 洞节点的右子节点
    while (rchild < len) {
...
inline void __pop_heap_aux(RandomAccessIterator first, RandomAccessIterator last,
	...
	*result = *first;  // 先将尾指设置成首值，即尾指为欲求结果
	___adjust_heap(first, static_cast<Distance>(0), last - first, value, comp);
}

template <class RandomAccessIterator, class T, class Compared>
inline void ___pop_heap(RandomAccessIterator first, RandomAccessIterator last, T*, Compared comp) {
...
inline void pop_heap(RandomAccessIterator first, RandomAccessIterator last, Compared comp) {
	___pop_heap(first, last, value_type(first), comp);
}
...
void __make_heap(RandomAccessIterator first, RandomAccessIterator last,
	...
	while (true) {
		// 重排以 holeIndex 为首的子树
		___adjust_heap(first, holeIndex, len, static_cast<T>(*(first + holeIndex)), comp);
		if (holeIndex == 0)  return;  // 走完根节点
		holeIndex--;                  // 向前移一个节点
	}
}
