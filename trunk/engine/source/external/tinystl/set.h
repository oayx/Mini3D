...
// 模板类 set
// 参数一代表键值类型，参数二代表键值比较方式，缺省使用 tinystl 的 less 
// 参数三代表空间配置器类型，缺省使用 tinystl 的 alloc
// 键值与实值相同，键值不允许重复，以 rb_tree 作为底层机制，元素会根据键值大小自动排序
template <class Key, class Compare = tinystl::less<Key>, class Alloc = alloc>
class set {
  public:
  ...
  private:
    // 以 rb_tree 作为底层机制
    typedef rb_tree<key_type, value_type, tinystl::identity<value_type>, key_compare, Alloc>  rep_type;
