...
// 模板类: vector
// 接受两个参数，参数一代表数据类型，参数二代表空间配置器类型，缺省使用 tinystl 的 alloc
...
vector<T, Alloc>& vector<T, Alloc>::operator=(const vector& rhs) {
    if (this != &rhs) {
        ...
        if (xlen > capacity()) {    // 如果要赋值的 vector 大小超过原 vector 容量大小
            ...
            tinystl::destroy(i, finish_);
            finish_ = start_ + xlen;
        }
        else {                      // 如果原 vector 大小小于要赋值的 vector 大小
            tinystl::copy(rhs.begin(), rhs.begin() + size(), start_);
            tinystl::uninitialized_copy(rhs.begin() + size(), rhs.end(), finish_);
...
void vector<T, Alloc>::reserve(size_type n) {
    if (capacity() < n) {
        ...
        tinystl::uninitialized_copy(start_, finish_, tmp);
        __destroy_and_deallocate();
        start_ = tmp;
...
void vector<T, Alloc>::push_back(const value_type& value) {
    if (finish_ != end_of_storage_) {
        tinystl::construct(finish_, value);
        ++finish_;
    }
    else {
...
vector<T, Alloc>::erase(iterator position) {
    if (position + 1 != end())
        tinystl::copy(position + 1, finish_, position);  // 后面的元素往前移
    --finish_;
    tinystl::destroy(finish_);
    return position;
}
...
vector<T, Alloc>::erase(iterator first, iterator last) {
    auto i = tinystl::copy(last, finish_, first);  // 把 last 之后的元素复制到 first 为起始的空间
    tinystl::destroy(i, finish_);                  // 销毁不要的元素
...
void vector<T, Alloc>::swap(vector<T, Alloc>& rhs) {
    if (this != &rhs) {
        ...
        tinystl::swap(end_of_storage_, rhs.end_of_storage_);
    }
}
...
void vector<T, Alloc>::__fill_initialize(size_type n, const value_type& value) {
    ...
    finish_ = tinystl::uninitialized_fill_n(start_, n, value);
    end_of_storage_ = finish_;
}
...
void vector<T, Alloc>::__fill_assign(size_type n, const value_type& value) {
    if (n > capacity()) {
        ...
        finish_ = tinystl::uninitialized_fill_n(finish_, n - size(), value);
    }
    else {
        erase(tinystl::fill_n(start_, n, value), finish_);
    }
}
...
void vector<T, Alloc>::__range_assign(ForwardIterator first, ForwardIterator last, forward_iterator_tag) {
    ...
    if (static_cast<size_type>(len) > capacity()) {               // 如果区间长度大于容器容量
        ...
        auto new_finish = tinystl::copy(first, last, start_);       // 复制区间到起始处
        tinystl::destroy(new_finish, finish_);                      // 销毁多余的元素
        finish_ = new_finish;
    }
    else {                                                        // 如果区间长度大于容器大小并且小于容器容量
        auto mid = first ;
        advance(mid, size());                                     // 以容器大小把区间分为前后段
        tinystl::copy(first, mid, start_);                          // 先复制前一段区间
        finish_ = tinystl::uninitialized_copy(mid, last, finish_);  // 再复制后一段区间
    }
}
...
void vector<T, Alloc>::__insert_aux(iterator position, const value_type& value) {
    if (finish_ != end_of_storage_) {                              // 如果还有备用空间
        ...
        ++finish_;
        tinystl::copy_backward(position, finish_ - 2, finish_ - 1);  // 后移一位
        ...
        *position = value_type;                                    // 将 position 位置元素修改为新值
        ...
        try {
            ...
            new_finish = tinystl::uninitialized_copy(start_, position, new_start);
            tinystl::construct(new_finish, value);                   // 插入新元素
            ++new_finish;
            // 将剩余元素拷贝到新的 vector
            new_finish = tinystl::uninitialized_copy(position, finish_, new_finish);
        }
        catch (...) {
...
void vector<T, Alloc>::__insert_dispatch(iterator position, InputIterator first,
    ...
    __range_insert(position, first, last, tinystl::iterator_category(first));
}
    ...
    if (n != 0) {
        if (static_cast<size_type>(end_of_storage_ - finish_) >= n) {
            ...
            if (static_cast<size_type>(after_elems) > n) {                          // 插入点后元素较多
                tinystl::uninitialized_copy(finish_ - n, finish_, finish_);           // 复制原元素
                finish_ += n;
                tinystl::copy_backward(position, old_finish - n, old_finish);         // 复制剩余元素
                tinystl::fill_n(position, n, value_copy);                             // 填充新元素
            }
            else {
                tinystl::uninitialized_fill_n(finish_, n - after_elems, value_copy);  // 填充新元素
                finish_ += n - after_elems;
                tinystl::uninitialized_copy(position, old_finish, finish_);           // 复制原元素
                finish_ += after_elems;
                tinystl::fill(position, old_finish, value_copy);                      // 填充新元素
            }
        }
        else {                                                                      // 备用空间不足
            // 新长度在 旧长度的两倍，旧长度 + 新增元素个数 中取较大值
            const auto old_size = size();
            const auto len = old_size + tinystl::max(old_size, n);
    ...
    if (first != last) {
        ...
        if ((end_of_storage_ - finish_) >= n) {           // 如果备用空间大小足够
            ...
            if (after_elems > n) {                        // position 后面的元素较多
                ...
                tinystl::uninitialized_copy(finish_ - n, finish_, finish_);
                finish_ += n;
                tinystl::copy_backward(position, old_finish - n, old_finish);
                tinystl::copy(first, last, position);       // 将要插入的元素复制到 position 处
            }
                ...
                tinystl::uninitialized_copy(position, old_finish, finish_);
                finish_ += after_elems;
                tinystl::copy(first, mid, position);        // 复制前半段到 position 位置
            }
        }
        else {                                            // 备用空间不足
            // 新长度在 旧长度的两倍，旧长度 + 新增元素个数 中取较大值
            const auto old_size = size();
            const auto len = old_size + tinystl::max(old_size, static_cast<size_type>(n));
            auto new_start = data_allocator::allocate(len);
            auto new_finish = new_start;
            try {
