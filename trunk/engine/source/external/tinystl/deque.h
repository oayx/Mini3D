...
deque<T, Alloc, BufSiz>::deque(const deque& rhs) {
    ...
    tinystl::uninitialized_copy(rhs.begin(), rhs.end(), start_);
}
...
deque<T, Alloc, BufSiz>& deque<T, Alloc, BufSiz>::operator=(const deque& rhs) {
    if (this != &rhs) {
        const auto len = size();
        if (len >= rhs.size()) {
            erase(tinystl::copy(rhs.begin(), rhs.end(), start_), finish_);
        }
        else {
            const_iterator mid = rhs.begin() + static_cast<difference_type>(len);
            tinystl::copy(rhs.begin(), mid, start_);
...
typename deque<T, Alloc, BufSiz>::iterator deque<T, Alloc, BufSiz>::erase(iterator position) {
    ...
    if (static_cast<size_type>(elems_before) < (size() >> 1)) {  // 清除点之前的元素比较少
        tinystl::copy_backward(start_, position, next);
        pop_front();
    }
    else {
        tinystl::copy(next, finish_, position);
        pop_back();
    }
...
typename deque<T, Alloc, BufSiz>::iterator deque<T, Alloc, BufSiz>::erase(iterator first, iterator last) {
    if (first == start_ && last == finish_) {  // 清空
        ...
        if (static_cast<size_type>(elems_before) < ((size() - len) >> 1)) {  // 清除区间前的元素较少
            ...
            tinystl::destroy(start_, new_start);                               // 销毁多余元素
            ...
            tinystl::destroy(new_finish, finish_);                    //销毁多余元素 
...
void deque<T, Alloc, BufSiz>::clear() {
    ...
    for (map_pointer cur = start_.node + 1; cur < finish_.node; ++cur) {
        tinystl::destroy(*cur, *cur + buffer_size());
        data_allocator::deallocate(*cur, buffer_size());
    }
    if (start_.node != finish_.node) {  // 有两个以上的缓冲区
        tinystl::destroy(start_.cur, start_.last);
        tinystl::destroy(finish_.first, finish_.cur);
        // 释放尾部的缓冲区，保留头部
        data_allocator::deallocate(finish_.first, buffer_size());
    }
    else {
        tinystl::destroy(start_.cur, finish_.cur);
        // 保留唯一的缓冲区
    }
    finish_ = start_;
}
...
void deque<T, Alloc, BufSiz>::push_back(const value_type& value) {
    if (finish_.cur != finish_.last - 1) {
        tinystl::construct(finish_.cur, value);
        ++finish_.cur;
    }
    else {
...
void deque<T, Alloc, BufSiz>::push_front(const value_type& value) {
    if (start_.cur != start_.first) {
        tinystl::construct(start_.cur - 1, value);
        --start_.cur;
    }
    else {
...
void deque<T, Alloc, BufSiz>::__map_initialize(size_type nElem) {
    ...
    map_size_ = tinystl::max(static_cast<size_type>(__initial_map_size), nNode + 2);
...
void deque<T, Alloc, BufSiz>::__range_initialize(ForwardIterator first,
    ...
    try {
        for (cur = start_.node; cur < finish_.node; ++cur) {
            ...
            tinystl::uninitialized_copy(first, mid, *cur);
            first = mid;
        }
...
void deque<T, Alloc, BufSiz>::__fill_insert(iterator position, size_type n, const value_type& value) {
    if (position.cur == start_.cur) {
        ...
        try {
            tinystl::uninitialized_fill(new_start, start_, value);
            start_ = new_start;
        }
        catch (...) {
        ...
        try {
            tinystl::uninitialized_fill(finish_, new_finish, value);
            finish_ = new_finish;
        }
        catch (...) {
    ...
    if (static_cast<size_type>(elems_before) < (len >> 1)) {
        ...
        try {
            if (static_cast<size_type>(elems_before) >= n) {
                ...
                start_ = new_start;
                tinystl::copy(start_n, position, old_start);
                tinystl::fill(position - n, position, value_copy);
            }
            else {
                tinystl::uninitialized_fill(tinystl::
                    uninitialized_copy(start_, position, new_start), start_, value_copy);
                start_ = new_start;
                tinystl::fill(old_start, position, value_copy);
            }
        }
        catch (...) {
            __destroy_node(new_start.node, start_.node);
        ...
        try {
            if (static_cast<size_type>(elems_after) > n) {
                ...
                finish_ = new_finish;
                tinystl::copy_backward(position, finish_n, old_finish);
                tinystl::fill(position, position + n, value_copy);
            }
            else {
                tinystl::uninitialized_fill(finish_, position + n, value_copy);
                tinystl::uninitialized_copy(position, finish_, position + n);
                finish_ = new_finish;
                tinystl::fill(position, old_finish, value_copy);
            }
        }
        catch (...) {
            __destroy_node(finish_.node + 1, new_finish.node + 1);
    ...
    if (static_cast<size_type>(elems_before) < (len >> 1)) {
        ...
        try {
            if (static_cast<size_type>(elems_before) >= n) {
                ...
                tinystl::uninitialized_copy(first, mid,
                    tinystl::uninitialized_copy(start_, position, new_start));
                start_ = new_start;
        ...
        try {
            if (static_cast<size_type>(elems_after) > n) {
                ...
                tinystl::uninitialized_copy(position, finish_,
...
void deque<T, Alloc, BufSiz>::__insert_dispatch_aux(iterator position,
    ...
    tinystl::copy(first, last, iterator(*this, position));
}
...
void deque<T, Alloc, BufSiz>::__push_back_aux(const value_type& value) {
    ...
    *(finish_.node + 1) = data_allocator::allocate(buffer_size());
    try {
        tinystl::construct(finish_.cur, value_copy);
...
void deque<T, Alloc, BufSiz>::__push_front_aux(const value_type& value) {
    ...
    *(start_.node - 1) = data_allocator::allocate(buffer_size());
    try {
        ...
        tinystl::construct(start_.cur, value_copy);
    }
    catch (...) {
        ++start_;
...
void deque<T, Alloc, BufSiz>::__reallocate_map(size_type node_to_add, bool add_at_front) {
    ...
    if (map_size_ > 2 * new_nodes_num) {
        ...
        if (new_start < start_.node)
            tinystl::copy(start_.node, finish_.node + 1, new_start);
        else
            tinystl::copy_backward(start_.node, finish_.node + 1, new_start + old_nodes_num);
    }
    else {
        auto new_map_size = map_size_ + tinystl::max(map_size_, node_to_add) + 2;
