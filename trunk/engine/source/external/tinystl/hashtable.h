...
// 参数四代表取出键值的方式，缺省使用 tinystl 的 identity
// 参数五代表键值比较方式，缺省使用 tinystl 的 equal_to，参数六代表空间配置器类型，缺省使用 tinystl 的 alloc
// hashtable 内的元素不会自动排序
template <class Val, class Key, class HashFcn, class ExtractKey = tinystl::identity<Key>, 
...
void hashtable<Val, Key, HashFcn, ExtractKey, EqualKey, Alloc>::swap(hashtable& rhs) {
    ...
    tinystl::swap(get_key_, rhs.get_key_);
    buckets_.swap(rhs.buckets_);
    tinystl::swap(element_nums_, rhs.element_nums_);
}
...
typename hashtable<Val, Key, HashFcn, ExtractKey, EqualKey, Alloc>::node* 
    ...
    tmp->next = nullptr;
    try {
        tinystl::construct(&tmp->value, value);
    }
    catch (...) {
        __put_node(tmp);
    }
