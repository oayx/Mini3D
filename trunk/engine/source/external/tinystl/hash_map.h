...
// 模板类 hash_map
// 参数一代表键值类型，参数二代表实值类型，参数三代表哈希函数，缺省使用 tinystl 的 hash
// 参数四代表键值比较方式，缺省使用 tinystl 的 equal_to，参数五代表空间配置器类型，缺省使用 tinystl 的 alloc
// 使用方法与 map 类似，使用 hashtable 作为底层机制，所以 hash_map 内的元素不会自动排序
template <class Key, class T, class HashFcn = tinystl::hash<Key>,
    class EqualKey = tinystl::equal_to<Key>, class Alloc = alloc>
class hash_map {
  private:
    // 使用 hash_table 作为底层机制
    typedef hashtable<tinystl::pair<const Key, T>, Key, HashFcn, 
        tinystl::selectfirst<tinystl::pair<const Key, T>>, EqualKey, Alloc>  rep_type;
    rep_type ht_;
...
class hash_multimap {
  private:
    ...
    typedef hashtable<pair<const Key, T>, Key, HashFcn,
        tinystl::selectfirst<pair<const Key, T>>, EqualKey, Alloc>  rep_type;
    rep_type ht_;
