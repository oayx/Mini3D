...
// 模板类 basic_string
// 参数一代表字符类型，参数二代表萃取字符类型的方式，缺省使用 tinystl 的 __char_type
// 参数三代表空间配置器，缺省使用 tinystl 的 alloc
...
class basic_string {
  ...
  private:
    const size_type    init_size = 8;     // 默认初始化长度
    const value_type   init_char = 0x20;  // 默认初始化字符
...
typename basic_string<CharType, CharTraits, Alloc>::iterator 
    ...
    if (pos == end()) {                                  // 插入位置在尾部
        finish_ = tinystl::fill_n(end(), count, ch);
        return pos + count;
    }
    ...
    if (static_cast<size_type>(elems_after) > count) {   // pos 后的元素较多
        finish_ = tinystl::uninitialized_copy(finish_ - count, finish_, finish_);
        tinystl::copy_backward(pos, old_finish - count, old_finish);
        tinystl::fill_n(pos, count, ch);
    }
    else {
        finish_ = tinystl::uninitialized_fill_n(finish_, count - elems_after, ch);
        finish_ = tinystl::uninitialized_copy(pos, old_finish, finish_);
        tinystl::fill(pos, old_finish, ch);
    }
    return pos + count;
}
...
typename basic_string<CharType, CharTraits, Alloc>::iterator 
    ...
    tinystl::copy_backward(pos, finish_, finish_ + 1);
    ++finish_;
    *pos = ch;
    return pos + 1;
}
...
void basic_string<CharType, CharTraits, Alloc>::insert(iterator pos,
    ...
    if (pos == end()) {                      // 插入位置在尾部
        finish_ = tinystl::copy(first, last, end());
        return;
    }
    const auto elems_after = finish_ - pos;  // pos 后的元素数目
    auto old_finish = finish_;
    if (elems_after > len) {                 // pos 后元素较多
        finish_ = tinystl::uninitialized_copy(finish_ - len, finish_, finish_);
        tinystl::copy_backward(pos, old_finish - len, old_finish);
...
typename basic_string<CharType, CharTraits, Alloc>::iterator 
    ...
    auto new_finish = tinystl::copy(pos + count, finish_, pos);
...
void basic_string<CharType, CharTraits, Alloc>::clear() {
    tinystl::destroy(buffer_, finish_);
    finish_ = buffer_;
}
...
basic_string<CharType, CharTraits, Alloc>
    ...
    count = tinystl::min(count, length() - index);
...
typename basic_string<CharType, CharTraits, Alloc>::pointer 
    ...
    tinystl::uninitialized_copy(buffer_, finish_, tmp);
    tmp[length()] = '\0';
    return tmp;
}
...
void basic_string<CharType, CharTraits, Alloc>::__initialize_string(size_type n, value_type ch) {
    auto len = tinystl::max(init_size, n);
    buffer_ = __get_buffer(len);
    finish_ = tinystl::uninitialized_fill_n(buffer_, n, ch);
    end_ = buffer_ + len;
}
...
void basic_string<CharType, CharTraits, Alloc>::__copy_from(const_pointer src,
    ...
    auto len = tinystl::max(init_size, count);
    buffer_ = __get_buffer(len);
    finish_ = tinystl::copy_n(src + pos, count, buffer_).second;
    end_ = buffer_ + len;
}
...
typename basic_string<CharType, CharTraits, Alloc>::iterator
    ...
    tinystl::uninitialized_copy(pos, finish_, 
        tinystl::uninitialized_fill_n(
            tinystl::uninitialized_copy(buffer_, pos, new_buffer), n, ch));
    __put_buffer(buffer_);
    buffer_ = new_buffer;
...
typename basic_string<CharType, CharTraits, Alloc>::iterator
    ...
    auto new_buffer = __get_buffer(new_len);
    tinystl::uninitialized_copy(pos, finish_,
        tinystl::uninitialized_copy(first, last,
            tinystl::uninitialized_copy(buffer_, pos, new_buffer)));
    __put_buffer(buffer_);
    buffer_ = new_buffer;
