...
struct __rb_tree_iterator :public __rb_tree_iterator_base {
	...
	bool operator==(const __rb_tree_iterator& rhs) { return this->node == rhs.node; }
	bool operator!=(const __rb_tree_iterator& rhs) { return !(this->node == rhs.node); }
};
...
// 模板类 rb_tree
// 参数一代表键值类型，参数二代表实值类型，参数三代表实值转化成键值的方式，缺省使用 tinystl 的 identity
// 参数四代表键值比较的方法，缺省使用 tinystl 的 less，参数代表为空间配置器的类型，缺省使用 tinystl 的 alloc
template <class Key, class Value, class KeyOfValue = tinystl::identity<Key>,
    class Compare = tinystl::less<Key>, class Alloc = alloc>
class rb_tree {
  private:
    typedef __rb_tree_node_base*    base_ptr;
...
void rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::swap(rb_tree& rhs) {
    ...
    tinystl::swap(key_compare, rhs.key_compare);
}
