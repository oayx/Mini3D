...
template <class T, class Sequence = tinystl::deque<T>>
class queue {
  public:
    // 使用底层容器的型别
  ...
  public:
    ...
    void            swap(queue& rhs)                    { tinystl::swap(s_, rhs.s_); }
...
// 参数三代表比较权值的方式，缺省使用 tinystl 的 less 作为比较方式
template <class T, class Sequence = tinystl::vector<T>, class Compare = tinystl::less<T>>
class priority_queue {
  ...
  public:
    ...
    explicit priority_queue(size_type n) :s_(n) {
        tinystl::make_heap(s_.begin(), s_.end(), comp_);
    }
  ...
  public:
    ...
    void push(const value_type& value) {
        try {
            s_.push_back(value);
            tinystl::push_heap(s_.begin(), s_.end(), comp_);
        }
        catch (...) { s_.clear(); }
    }
    void pop() {
        try {
            tinystl::pop_heap(s_.begin(), s_.end(), comp_);
            s_.pop_back();
        }
        catch (...) { s_.clear(); }
    }
    ...
    void clear()                   { while (!empty())    pop(); }
    void swap(priority_queue& rhs) { tinystl::swap(s_, rhs.s_); tinystl::swap(comp_, rhs.comp_); }
};
