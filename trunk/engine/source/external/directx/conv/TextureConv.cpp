...
std::wstring ToWString(char* ch)
{
	std::wstring val = L"";
	size_t size_of_wc;
	size_t destlen = mbstowcs(0, ch, 0);
	if (destlen == (size_t)(-1))
		return val;

	size_of_wc = destlen + 1;
	wchar_t * pw = new wchar_t[size_of_wc];
	destlen = mbstowcs(pw, ch, size_of_wc);
	if (destlen != (size_t)(-1))
		val = pw;
...
#endif
int DDSConvertTexture(std::vector<std::wstring> args)
{
    ...
    {
        PWSTR pArg = const_cast<PWSTR>(args[iArg].c_str());
        ...
        if (('-' == pArg[0]) || ('/' == pArg[0]))
        {
            ...
            case OPT_SWIZZLE:
                ...
                if (!*pValue)
                {
                    ...
                    iArg++;
					pValue = const_cast<PWSTR>(args[iArg].c_str());
                }
                break;
            }
...
int dxt_main(int argc, char** argv)
{
	std::vector<std::wstring> list;
	for (int iArg = 0; iArg < argc; iArg++)
	{
		std::wstring wstr = ToWString(argv[iArg]);
		list.push_back(wstr);
	}
	return DDSConvertTexture(list);
}