...
void DX11RenderTexture::BeginFrame(RenderFrameDesc& desc)
{
	...
	desc.target_buffer = _colorRTV[desc.face];
	desc.depth_stencil_buffer = _depthDSV;
...
bool DX11RenderTexture::GetData(MemoryDataStream& stream)
{
	...
	HR(GetDX11Device()->GetContent()->Map(_stagingTexture, 0, D3D11_MAP_READ, 0, &mapped));
	stream.Resize(mapped.RowPitch * _imageHeight);
	Memory::Copy(stream.data(), mapped.pData, mapped.RowPitch * _imageHeight);
...
{
	{
		...
		tex_desc.Format = DX11GetTextureFormat(_imageFormat, false);
		...
		HR(GetDX11Device()->GetDevice()->CreateTexture2D(&tex_desc, NULL, &_colorTexture));
	}
	{
		D3D11_SHADER_RESOURCE_VIEW_DESC view_desc = {};
		view_desc.Format = DX11GetShaderViewFormat(_imageFormat, false);
		if (_textureType == TextureType::Cube)
		...
		HR(GetDX11Device()->GetDevice()->CreateShaderResourceView(_colorTexture, &view_desc, &_colorSRV));
	}
	{
		D3D11_RENDER_TARGET_VIEW_DESC rt_desc = {};
		rt_desc.Format = DX11GetTextureViewFormat(_imageFormat, false);
		if (_textureType == TextureType::Cube)
		...
		if (_textureType == TextureType::Cube)
		{
			for (int i = 0; i < 6; ++i)
			{
				...
				HR(GetDX11Device()->GetDevice()->CreateRenderTargetView(_colorTexture, &rt_desc, &_colorRTV[i]));
			}
		}
		else
		{
			HR(GetDX11Device()->GetDevice()->CreateRenderTargetView(_colorTexture, &rt_desc, &_colorRTV[0]));
		}
	}
	//mipmap
	if (_enableMips)
...
{
	{
		...
		depth_desc.Format = DX11GetDepthTextureFormat(_imageFormat);
		...
		HR(GetDX11Device()->GetDevice()->CreateTexture2D(&depth_desc, NULL, &_depthTexture));
	}
	{
		D3D11_SHADER_RESOURCE_VIEW_DESC view_desc = {};
		view_desc.Format = DX11GetDepthShaderViewFormat(_imageFormat);
		view_desc.ViewDimension = _antiAlias > 1 ? D3D11_SRV_DIMENSION_TEXTURE2DMS : D3D11_SRV_DIMENSION_TEXTURE2D;
		view_desc.Texture2D.MostDetailedMip = 0;
		view_desc.Texture2D.MipLevels = 1;
		HR(GetDX11Device()->GetDevice()->CreateShaderResourceView(_depthTexture, &view_desc, &_depthSRV));
	}
	{
		D3D11_DEPTH_STENCIL_VIEW_DESC depth_view_desc = {};
		depth_view_desc.Format = DX11GetDepthTextureViewFormat(_imageFormat);
		...
		HR(GetDX11Device()->GetDevice()->CreateDepthStencilView(_depthTexture, &depth_view_desc, &_depthDSV));
	}
}
