...
struct WindowResizeDesc
{
	...
	bool fullscreenValid = true;//fullscreen标记是否有效，如果无效，使用之前设定
	WindowResizeDesc() {}
	WindowResizeDesc(void* hwnd, int w, int h, bool full, bool valid) :hWnd(hwnd), width(w), height(h), fullscreen(full), fullscreenValid(valid) {}
};
...
struct RenderFrameDesc
{
	...
	void* targetBuffer = nullptr;
	void* depthStencilBuffer = nullptr;
	ClearFlag clearFlag = ClearFlag::DonotClear;
