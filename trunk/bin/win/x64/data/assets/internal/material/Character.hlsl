...
v2f vs_main(appdata v)
{
	...
	pos += mul(BoneMatrixArray[(int)v.indices[0]], v.vertex) * v.weights[0];
	pos += mul(BoneMatrixArray[(int)v.indices[1]], v.vertex) * v.weights[1];
	pos += mul(BoneMatrixArray[(int)v.indices[2]], v.vertex) * v.weights[2];
	pos += mul(BoneMatrixArray[(int)v.indices[3]], v.vertex) * v.weights[3];
