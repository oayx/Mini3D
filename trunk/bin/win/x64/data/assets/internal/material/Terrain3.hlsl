...
DECLARE_TEXTURE(_MainTex2, 4);
DECLARE_TEXTURE(_NormalTexture2, 5);
DECLARE_TEXTURE(_ControlTex, 6);
...
struct v2f
{
	...
	float3 world_light_dir : TEXCOORD7;
	float3 world_view_dir : TEXCOORD8;
	FOG_COORDS(9)
};
...
float4 ps_main(v2f i) : SV_Target
{
	...
	float4 layer2 = SAMPLE_TEXTURE(_MainTex2, i.uv2);
	float4 layer_control = SAMPLE_TEXTURE(_ControlTex, i.uv_control);
	float4 albedo = float4(layer0.rgb * layer_control.r + layer1.rgb * layer_control.g + layer2.rgb * layer_control.b, 1.0);
	...
	float3 normal2_w = UnpackNormal(SAMPLE_TEXTURE(_NormalTexture2, i.uv2));
	normal0_w = normalize(mul(normal0_w, i.rotation));
	normal1_w = normalize(mul(normal1_w, i.rotation));
	normal2_w = normalize(mul(normal2_w, i.rotation));
	float3 world_normal = normal0_w * layer_control.r + normal1_w * layer_control.g + normal2_w * layer_control.b;
